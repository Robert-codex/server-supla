...
abstract class SuplaAutodiscover {
    ...
    public function __construct(
        ...
        $this->autodiscoverUrl = is_string($autodiscoverUrl) ? trim($autodiscoverUrl) : '';
        ...
        if ($this->enabled() && !str_starts_with($this->autodiscoverUrl, 'http')) {
    ...
    public function getAutodiscoverUrl(): ?string {
        return $this->enabled() ? $this->autodiscoverUrl : null;
    }
    ...
    public function issueRegistrationTokenForTargetCloud(TargetSuplaCloud $targetCloud, $email): string {
        ...
        if ($responseStatus !== 201) {
            $errors = [
                409 => 'Private instance of the SUPLA cloud with given URL is already registered.',
    ...
    public function issueRemovalTokenForTargetCloud(TargetSuplaCloud $targetCloud, $email): array {
        ...
        if ($responseStatus !== 201) {
            $errors = [
                400 => 'Invalid request.',
                404 => 'Target Cloud is not registered, or is registered with another e-mail address.',
                503 => 'Could not contact Autodiscover service. Try again in a while.',
            ];
            throw new ApiException($errors[$responseStatus] ?? $errors[503], $responseStatus);
        }
    ...
    public function requestDeviceUnlockCode(IODevice $device, string $unlockerEmail): string {
        ...
        if ($responseStatus !== 200) {
            $errors = [
                404 => 'Invalid e-mail address.',
                409 => 'The device has been unlocked already.',
                503 => 'Could not contact Autodiscover service. Try again in a while.',
            ];
            throw new ApiException($errors[$responseStatus] ?? $errors[503], $responseStatus);
        }
    ...
    public function unlockDevice(IODevice $device, string $unlockCode): void {
        ...
        if ($responseStatus !== 200) {
            $errors = [
                400 => 'Invalid unlock code.',
                404 => 'Invalid unlock code.',
                503 => 'Could not contact Autodiscover service. Try again in a while.',
            ];
            throw new ApiException($errors[$responseStatus] ?? $errors[503], $responseStatus);
        }
