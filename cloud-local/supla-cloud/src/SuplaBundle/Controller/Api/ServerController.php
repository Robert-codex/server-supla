...
class ServerController extends RestController {
    ...
    public function getServerInfoAction(Request $request, RealClientIpResolver $clientIpResolver) {
        ...
        $extendedResult = $this->buildExtendedServerInfo($request, $clientIpResolver);
        if (ApiVersions::V2_2()->isRequestedEqualOrGreaterThan($request)) {
            $result = array_merge($result, $extendedResult);
        } else {
            // Keep the legacy {"data": ...} envelope for older clients, but expose
            // the same fields inside it so the unversioned webapp request gets the
            // cloud version and frontend config expected by the modern frontend.
            $result = ['data' => array_merge($result, $extendedResult)];
        }
    ...
    private function buildExtendedServerInfo(Request $request, RealClientIpResolver $clientIpResolver): array {
        $user = $this->getUser();
        $result = [
            'authenticated' => !!$user,
            'cloudVersion' => $this->suplaVersion,
            'cloudVersionFull' => $this->suplaVersionFull,
            'apiVersion' => ApiVersions::fromRequest($request)->getValue(),
            'supportedApiVersions' => array_values(array_unique(ApiVersions::toArray())),
            'config' => $this->frontendConfig->getConfig(),
