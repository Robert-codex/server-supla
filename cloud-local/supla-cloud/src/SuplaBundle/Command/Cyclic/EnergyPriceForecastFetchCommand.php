...
class EnergyPriceForecastFetchCommand extends AbstractCyclicCommand {
    ...
    protected function execute(InputInterface $input, OutputInterface $output) {
        ...
        if (!$this->isForecastCollection($forecast)) {
            return 0;
        }
        foreach ($forecast as $forecastItem) {
            if (!$this->isForecastItem($forecastItem)) {
                continue;
            }
    ...
    private function isForecastCollection(mixed $forecast): bool {
    ...
    private function isForecastItem(mixed $forecastItem): bool {
        return is_array($forecastItem)
            && isset($forecastItem['dateFrom'], $forecastItem['dateTo'])
            && is_string($forecastItem['dateFrom'])
            && is_string($forecastItem['dateTo']);
    }
}
